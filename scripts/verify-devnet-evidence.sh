...
require_regex "production allocation strategy record tamper rejection" 'test native::tests::production_allocation_strategy_record_round_trips_and_rejects_tamper \.\.\. ok'
require_regex "production allocation strategy record handoff" 'native allocation: production allocation strategy record handoff'
...
require_regex "allocation audit certificate" 'live: allocation audit certificate verified certificate_id=0x[0-9a-f]{64} proof_entries=6 vk_bytes_total=[0-9]+ proof_bytes_total=[0-9]+ canonical_bytes=[0-9]+'
