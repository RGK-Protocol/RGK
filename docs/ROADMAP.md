...
The roadmap now tracks RGK as a Kaspa-native client-side asset protocol
inspired by RGB. Matching external wallets or validators byte-for-byte is not
a milestone.

## Vocabulary Boundary

RGK keeps these design intuitions:
...
* receipt / consignment intuition
* resolver-based verification

These terms are being migrated into Kaspa-native protocol language:

| Legacy or borrowed wording | RGK-native wording |
| --- | --- |
| seal | covenant output / lane state output |
| seal close | covenant spend |
| next seal | continuation output |
| consignment | encrypted receipt transcript |
| schema | lane calculus |
| contract | lane policy / proof policy |
| commitment | Toccata covenant commitment + ZK public-input boundary |
| allocation history | private allocation transcript |
| amount disclosure | blinded amount commitment |
| resolver | DAG-aware lineage reconstruction |

The following are not RGK roadmap goals or public protocol language:

* AluVM / RGB VM baggage
* Bitcoin-specific Tapret/Opret dependence
* Bitcoin outpoint-seal worldview
* over-abstract strict-types universe
* standards-organisation naming theology
* manual consignment UX
* external-runtime equivalence as a goal
* cell terminology

The phrase "receipt / consignment intuition" is retained only as design
lineage. User-facing and protocol-facing flows use encrypted receipt
transcripts and DAG-aware lineage reconstruction.
...
| Argent equivalence | Out of RGK core scope; optional Kaspa-wide frontend equivalence work |
| Public testnet staging | Harness, preflight manifest, funding-address helper, and report verifiers live; funded public run open |
...
* State digest binds asset id, allocation root, policy commitment, privacy
  mode, and lane id.
...
* A bounded Groth16 lane-discovery circuit, accepted by the upstream Toccata
  VM, proves that a hidden view key and hidden asset id derive the public
  blinded lane id and rotating scan tag for a public epoch.
* A bounded Groth16 lane-graph discovery circuit proves an ordered set of
  public lane nodes and graph root under the same hidden view key and asset id,
  with upstream Toccata VM and local devnet evidence for a 2-node graph.
...
derivations for a bounded public epoch while keeping the view key and asset id
private, and is accepted by the upstream Toccata VM test harness.
...
asset id; the evidenced shape is a 2-node current/look-ahead graph accepted by
the upstream Toccata VM and required by local devnet evidence.
...
segment capacity, segment counts, exclusion-cell count, and proof-cell count.
Burns and empty sides fail closed on the segmented strategy. RGK still needs a
future recursive/aggregated strategy before it can claim arbitrary one-step
...
* DONE: Bounded private-lane discovery proves, inside Groth16 and the upstream
  Toccata VM, that a hidden view key and hidden asset id derive the public
  blinded lane id and rotating scan tag for a public epoch.
* DONE: Bounded private-lane graph discovery proves, inside Groth16 and the
  ...
  nodes are derived under one hidden view key and hidden asset id.
* DONE: Segmented private-lane graph discovery proves, inside Groth16 and the
  ...
  graph root under one hidden view key and hidden asset id; local devnet
  evidence requires a 2-segment / 4-node chain.
...
* DONE: Native wallet-facing production allocation strategy planning exposes
  ...
  transfers, binds a strategy commitment, and fails closed for segmented burns
  or empty allocation sides.
...
* Keep Argent equivalence outside RGK core scope. Argent may prove equivalent
  higher-level programs for the same examples, but Argent is a Kaspa-wide
  covenant frontend rather than an RGK-specific language.
* Stage on public testnet with persistent indexer recovery.
  ...
  prints the deterministic funding address and minimum value, and its
  `--preflight` mode writes a machine-checked funding manifest with a native
  preflight id before the funded public run.
...
* OPEN: Argent equivalence, when available, is tracked as external evidence and
  does not become an RGK-specific language dependency.
...
byte-identical digest matching with another asset protocol.
